...
func metrics(response http.ResponseWriter, request *http.Request) {
	out, err := exec.Command(
		"nvidia-smi",
		"--query-gpu=name,index,temperature.gpu,utilization.gpu,utilization.memory,memory.total,memory.free,memory.used,pstate,fan.speed,encoder.stats.sessionCount,encoder.stats.averageFps,encoder.stats.averageLatency,power.draw,power.limit,clocks.current.graphics,clocks.current.sm,clocks.current.memory,clocks.current.video",
	...
	metricList := []string{
		...
		"utilization.memory", "memory.total", "memory.free", "memory.used",
		"pstate", "fan.speed", "encoder.stats.sessionCount", "encoder.stats.averageFps",
		"encoder.stats.averageLatency", "power.draw", "power.limit", "clocks.current.graphics",
